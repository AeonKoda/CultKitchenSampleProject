class_name CardRecipe extends Card
static func get_type_id()-> StringName:return &"CardRecipe"
...
var result_resource:CardResource
...
func _ready() -> void:
	...
	texture.position = Vector2.ZERO # changing the size moves the texture -13 y, don'y know why
	...
	if card_resource and card_resource.extended_class_resource:
		var recipe_resource:Resource = card_resource.extended_class_resource
		if recipe_resource is not CardResource: 
			push_warning("Trying to load RecipeCard with non CardResource")
			return
		result_resource = recipe_resource
		result_name = result_resource.card_name
		occult = result_resource.occult
		
		#if not Global.game.is_node_ready():
			#await  Global.game_ready
		
		for i:int in result_resource.recipes.size():
			var recipes:Array = result_resource.recipes.duplicate()
			if result_name == "RITUAL_CIRCLE":
				recipes.reverse()
			
			var alt_recipe:Recipe = recipes[i]
			var new_recipe_label:RichTextLabel = recipe_label.duplicate()
			new_recipe_label.show()
			
			recipe_label.get_parent().add_child(new_recipe_label)
			new_recipe_label.text = make_recipe_text(alt_recipe)
			
			if result_name.ends_with("RITUAL"):
				title_label.text = str("[center]",result_name.replace("_"," "),"[/center]")
			else:
				title_label.text = str("[center]",result_name.replace("_"," ")," RECIPE","[/center]")
			
			if i > 0:
				var new_indicator:TextureButton = recipe_indicator.duplicate()
				new_indicator.button_pressed = false
				indicator_h_box.add_child(new_indicator)
				indicator_h_box.show()
...
func get_recipe() ->String:
	var recipe_text:String = ""
	if not result_resource.recipes.is_empty():
		var recipes:Recipe = result_resource.recipes[0]
