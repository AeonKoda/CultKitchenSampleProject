...
static func tick(card:Card) ->void:
	...
	if not card_resource.fuel_cards.is_empty():
		...
		if previous.card_resource not in card_resource.fuel_cards:return
		
		var previous_clicker:ClickerComponent = ClickerComponent.get_component_or_null(previous)
		if previous_clicker:
			previous_clicker.click()
	
	var card_slice:Array[Control] = stack_component.get_slice(0,RecipeBook.MAX_COMBINATION_SIZE)
	var resource_slice:Array = card_slice.map(func(obj:Control)->CardResource:return obj.get("card_resource"))
	...
	if result:
		...
		var reagents:Array = result["reagents"].map(func(index:int)->Control:return card_slice[index])
		...
		reagents.pop_front() # removes the tool from reagent list
