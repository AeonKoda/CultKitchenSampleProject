extends Script

static func click(card:Card,card_resource:Resource = card.card_resource) ->void:
	var stack_component:StackComponent = StackComponent.get_component_or_null(card)
	var guide_2:Card
	
	var ticker_component:TickerComponent = TickerComponent.get_component_or_null(card)
	if ticker_component:
		ticker_component.release()
	
	if not card_resource.spawn_cards.is_empty():
		var new_card:Card
		var new_resource:CardResource
		
		for spawn:CardResource in card_resource.spawn_cards:
			new_resource = spawn
			if card_resource.card_name in Global.land_cards:
				var previous_card:Card = stack_component.previous if stack_component and stack_component.previous is Card else null
				if previous_card:
					if previous_card.card_resource.card_name in Global.crop_cards:
						new_resource = previous_card.card_resource
					elif previous_card.card_resource.card_name in Global.spawn_crop_cards:
						new_resource = previous_card.card_resource.spawn_cards[0]
			
			new_card = Global.create_card(new_resource)
			card.get_parent().add_child(new_card)
			new_card.position = card.position #card.get_top().position + Global.game.board.CREATE_CARD_OFFSET
			
			if spawn.card_name == "GUIDE_2":
				guide_2 = new_card
			if spawn.card_name == "GUIDE_3":
				var ticker:TickerComponent = TickerComponent.get_component_or_null(new_card)
				ticker.tick_up()
				ticker.set_invisible(true)
			
			if stack_component:
				stack_component.append(new_card)

		
	var clicker_component:ClickerComponent = ClickerComponent.get_component_or_null(card)
	clicker_component.release()
	
	card.card_queued_free.connect(_on_card_releasing.bind(guide_2))



static func _on_card_releasing(_guide_1:Card,guide_2:Card)-> void:
	var ticker:TickerComponent = guide_2.create_ticker(guide_2.card_resource)
	ticker.tick_up()
	ticker.set_invisible(true)
	
